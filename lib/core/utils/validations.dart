...
class Validators {
  ...
  static final contactPattern = RegExp(
    r"^0\d{10}$"
  );
  ...
  static Validator contact() {
    return matchPattern(contactPattern, "contact");
  }

  static Validator emailOrContact(String value) {
    if(!value.contains(Validators.emailPattern) && value.contains(Validators.contactPattern)){
     return matchPattern(Validators.emailPattern, 'email or contact');
    }

    if(value.contains(Validators.emailPattern) && !value.contains(Validators.contactPattern)){
     return matchPattern(Validators.contactPattern, 'email or contact');
    }
